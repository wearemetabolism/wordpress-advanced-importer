 ...
 * }
 */
function advanced_export_wp( $args = array() ) {
	 ...
	 */
	do_action( 'advanced_export_wp', $args );
	...
	if( $args['content'] == 'options' ||  'all' == $args['content'] ) {

		$raw_options = wp_load_alloptions();

		foreach ($raw_options as $name => $value) {
			if (!isset($args['filter']) || strpos($name, $args['filter']) === 0 || strpos($name, '_' . $args['filter']) === 0) {

				if (!in_array($name, ['siteurl', 'home', 'admin_email']))
					$options[] = ['name' => $name, 'value' => $value];
			}
		}

		$options = apply_filters('export_wp_options', $options);
	}
	...
	>
		...
		<channel>
			...
			<?php foreach ( $options as $option ) : ?>
				<wp:options>
					<wp:name><?=$option['name']?></wp:name>
					<wp:value><?=wxr_cdata( $option['value'] )?></wp:value>
				</wp:options>
			<?php endforeach; ?>
				...
				while ( $next_posts = array_splice( $post_ids, 0, 20 ) ) {
					...
					foreach ( $posts as $post ) {
						...
						<item>
							...
							$postmeta = apply_filters('postmeta_export', $postmeta);
