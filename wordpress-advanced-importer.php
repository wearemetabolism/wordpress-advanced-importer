...
require dirname( __FILE__ ) . '/export-options.php';
...
if ( class_exists( 'WP_Importer' ) ) {
	 ...
	 * WP_AdvancedImport
	 */
	class WP_AdvancedImport extends WP_Importer {
		 ...
		 * @return int|WP_Error
		 */
		function fetch_attachment( $value ) {

			$url = str_replace('attachment://', '/', $value);
			...
			return $this->process_attachment($post, $url);
		}
				...
				if ( ! empty( $post['postmeta'] ) ) {
					foreach ( $post['postmeta'] as $meta ) {
						...
						if ( $key ) {
							...
							$value = apply_filters( 'import_post_meta_value', $value, $post_id, $post );
		...
		function greet() {
			...
			echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, tags and options into this site.', 'wordpress-importer' ).'</p>';
	...
	add_action( 'admin_init', function() {
		 ...
		 * @global WP_AdvancedImport $wp_import
		 */
		$GLOBALS['wp_import'] = new WP_AdvancedImport();

		register_importer( 'wordpress', 'WordPress', __('Import <strong>posts, pages, comments, custom fields, categories, tags and options</strong> from a WordPress export file.', 'wordpress-importer'), array( $GLOBALS['wp_import'], 'dispatch' ) );
	});
